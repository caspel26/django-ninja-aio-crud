...
class BaseManyToManyAPITestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.rel_util = ModelUtil(cls.related_model, serializer_class=cls.serializer_class)
    ...
    async def test_get_related_with_filters(self):
        ...
        await self._add_related()
        filters_schema = self.viewset.m2m_api.relations_filters_schemas[self.related_name]
...
class ManyToManyAPITestCase(BaseManyToManyAPITestCase):
...
class M2MRelationSchemaSerializerClassTestCase(BaseManyToManyAPITestCase):
