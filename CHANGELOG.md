...
#### 📑 Consistent paginated response shape for M2M GET endpoints
> `ninja_aio/helpers/api.py`

`ManyToManyAPI._register_get_relation_view` previously delegated pagination to the `@paginate(self.pagination_class)` decorator from `ninja.pagination`. That decorator wraps the response in an auto-generated `Paged{Schema}` model, which diverged from the main `list_view` endpoint in `views/api.py`, where the response schema is built explicitly as `Paginated{Schema}` with `items` and `count` fields.

The GET relation handler now mirrors `list_view` exactly:

- A custom `Paginated{related_schema.__name__}` schema is built via `pydantic.create_model`, with `items: List[related_schema]` and `count: int`.
- The `@paginate(...)` decorator is removed; pagination is applied manually using `view_set._get_page_params(...)` and queryset slicing.
- `count` is computed once via `await related_qs.acount()` before slicing, after filters are applied.
- Filtering via `{related_name}_query_params_handler` continues to run before pagination.

```python
# Before — response shape generated by ninja's @paginate:
# {
#   "items": [...],
#   "count": 42
# }
# Schema name: PagedRelatedSchema

# After — explicit, framework-controlled shape:
# {
#   "items": [...],
#   "count": 42
# }
# Schema name: PaginatedRelatedSchema
```

The wire-level shape (`items` + `count`) is unchanged; the OpenAPI schema name now matches the convention used by the main list endpoint.

---

### 🎯 Summary

Patch release aligning the response schema naming for `ManyToManyAPI` GET-related endpoints with the rest of the framework. M2M GET responses are now described as `Paginated{Schema}` instead of `Paged{Schema}`, removing a small but visible inconsistency in generated OpenAPI specs.

**Key benefits:**

- ✅ Uniform `Paginated{Schema}` naming across all paginated list endpoints (top-level and M2M)
- ✅ Pagination logic unified — single code path between `list_view` and the M2M GET handler
- ✅ No wire-format change: clients receive the same `{items, count}` payload
- ✅ No behavioral change for filters, query handlers, or auth — only the response schema name and code path
