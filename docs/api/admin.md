...
When registration happens during model loading, field and relation discovery
are deferred until Django's model registry is ready. This includes children
declared later in the module and reverse relations listed in read fields.
...
- Uses editable model fields from the child's `UpdateSerializer.fields` and
  `optionals`, falling back to the create config if no editable fields remain.
  Computed inputs, primary keys, noneditable fields, and custom-through M2M
  fields are excluded.
- Always drops the FK field pointing back at the parent (`book` above); it
  is supplied via Django's inline formset (`fk_name`). Multiple FKs from the
  same child to the parent get distinct inline classes and explicit `fk_name`.
