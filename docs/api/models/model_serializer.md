...
| `customs` | `list[tuple[str, type, Any]]` | Synthetic input fields: `(name, type, default)` |
...
1. Value from incoming payload
2. If `default` is callable → invoke it
3. Use literal `default` value
...
| `excludes` | `list[str]` | Field names to force-exclude (e.g., passwords) |
| `customs` | `list[tuple[str, type, Any]]` | Computed attributes: `(name, type, default)` |
...
**Resolution Order for `customs`:**
1. Property/attribute on model instance
2. Callable (invoke with object)
3. Literal default value
...
| `customs` | `list[tuple[str, type, Any]]` | Instruction fields: `(name, type, default)` |
